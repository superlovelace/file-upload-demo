...
import com.demo.constants.ConsoleColorConstants;
 ...
 * @author peter
 * @date 2025/5/6
 * 注意：若要通过concurrentHashMap存储分片信息，请注释掉此类,并反注释掉IndexController
 */
@Slf4j
@Api(tags = "文件上传接口")
@CrossOrigin
...
public class IndexController2 {
    ...
    @Qualifier("fileUploadsServiceImpl2")
    ...
    @javax.annotation.Resource
    private UploadUtilsByRedis uploadUtilsByRedis;
    ...
    public ResponseEntity<String> progress(
        ...
        try {
            chunkInfo = uploadUtilsByRedis.getChunkInfo(sha256);
            log.debug(ConsoleColorConstants.GREEN_BOLD + "分块存储查询：{}" + ConsoleColorConstants.RESET, chunkInfo);
        } catch (NullPointerException e) {
            chunkInfo = Collections.emptyMap();
        }
        ...
        // 有上传中的文件
        String status = (String) chunkInfo.get("status");
        int count = 0;
    ...
    public Map<String, Value> getInfo(){
        return uploadUtilsByRedis.getChunkMapInfo();
    }
}
