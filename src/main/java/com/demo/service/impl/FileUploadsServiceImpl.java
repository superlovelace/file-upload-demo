...
import static com.demo.utils.UploadUtils.*;
...
public class FileUploadsServiceImpl implements FileUploadsService {
    ...
    public String uploadChunks(String name,
        ...
        // 生成存储文件名
        String fileName = UploadUtils.getFileName(sha236, chunks);
        // 存储文件路径
        ...
        // 保存分片上传状态
        addChunk(sha236,chunk);
        // 判断所有分片是否已上传
        if (isUploaded(sha236)) {
            // 删除分块记录信息
            removeKey(sha236);
            ...
            fileUploads.setSha256Hash(sha236);
